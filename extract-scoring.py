...
for path in sys.argv[1:]:
    ...
    with open(path) as fd:
        src = fd.read()
        url = re.search('<link rel="canonical" href="(.*?)" />', src, re.DOTALL).group(1).split('/')[-1]
        ...
        outstr = json.dumps(result) # need to render to string to print out with newline in one go
        print(outstr)
